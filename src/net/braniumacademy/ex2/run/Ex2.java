package net.braniumacademy.ex2.run;

import net.braniumacademy.ex2.BankAccount;
...
public class Ex2 {
    private static final String fileName = "ACC.DAT";

    public static void main(String[] args) {
        int choice;
        ArrayList<BankAccount> accounts = new ArrayList<>(readFromFile());
        if (accounts.size() > 0) {
            setNextId(accounts.get(accounts.size() - 1).getId());
        }
        var format = "dd/MM/yyyy";
        SimpleDateFormat dateTimeFormat = new SimpleDateFormat(format + ", HH:mm:ss");
        ...
        do {
            ...
            switch (choice) {
                case 0 -> out.println("==> CẢM ƠN BẠN ĐÃ SỬ DỤNG DỊCH VỤ <==");
                case 1 -> {
                    var acc = createNewAcc(input);
                    accounts.add(acc);
                    out.println("==> Thêm tài khoản thành công! <==");
                }
                case 2 -> showAccounts(accounts);
                case 3 -> {
                    if (accounts.size() > 0) {
                        ...
                        if (isSuccess) {
                            ...
                            out.printf("==> Thời gian giao dịch: %s <==\n",
                    ...
                    if (accounts.size() > 0) {
                        ...
                        if (result != null) {
                            out.println("==> Tìm thấy tài khoản mã "
                                    + id + " <==");
                            showAcc(result, new SimpleDateFormat(format));
                        } else {
                            out.println(
                ...
                case 9 -> {
                    if (accounts.size() > 0) {
                        ...
                        if (isDeleted) {
                            ...
                            out.printf("==> Thời gian thực hiện: %s <==\n",
                                    dateTimeFormat.format(new Date()));
                        } else {
                ...
                default -> out.println("==>Sai chức năng, vui lòng chọn lại <==");
            }
        } while (choice != 0);
    }
            ...
            for (BankAccount acc : accounts) {
                if (!isExist(currentInFile, acc)) {
                    printWriter.printf("%s-%s-%s-%s-%d-%s-%s-%s\n", acc.getId(), acc.getAccNumber(),
                            ...
                            dateFormat.format(acc.getStartDate()),
                            dateFormat.format(acc.getEndDate()));
                }
            }
            printWriter.close();
            fileWriter.close();
    ...
    private static ArrayList<BankAccount> readFromFile() {
        ...
        try {
            ...
            while (input.hasNextLine()) {
                ...
                var startDate = dateFormat.parse(accInfo[6]);
                var endDate = dateFormat.parse(accInfo[7]);
    ...
    private static BankAccount createNewAcc(Scanner input) {
        ...
        var format = "dd/MM/yyyy";
        SimpleDateFormat dateFormat = new SimpleDateFormat(format);
        out.println("Ngày phát hành dạng dd/MM/yyyy, ví dụ 20/12/2020: ");
        Date startDate = null;
        try {
        ...
        out.println("Ngày hết hạn dạng dd/MM/yyyy, ví dụ 20/11/2025: ");
        Date endDate = null;
        try {
        ...
        return new BankAccount(null, number, owner, type,
    ...
    private static BankAccount searchByAccNumber(
        ...
        for (BankAccount account : accounts) {
            if (account.getAccNumber().compareTo(accNum) == 0) {
                return account;
            }
        }
        return null;
    ...
    private static BankAccount searchByAccId(ArrayList<BankAccount>
        ...
        for (BankAccount account : accounts) {
            // nếu tìm thấy
            if (account.getId().compareToIgnoreCase(id) == 0) {
                return account; // trả về
            }
        }
     ...
     */
    private static void showAcc(BankAccount acc, SimpleDateFormat dateFormat) {
